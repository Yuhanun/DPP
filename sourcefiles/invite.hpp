...
discord::Invite::Invite(std::string const &invite_str) {
  ...
  auto guild =
      discord::utils::get(discord::bot_instance->guilds,
                          [temp_guild_id](std::unique_ptr<discord::Guild> &g) {
                            return g->id == temp_guild_id;
                          });

  snowflake temp_channel_id =
      std::stoul(data["channel"]["id"].get<std::string>());
