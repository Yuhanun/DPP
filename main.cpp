...
int main() {
    ...
    bot.register_command("test", [](discord::Context ctx) {
        std::string output =
            discord::format("%, these were your arguments: ", ctx.user->mention);
        for (auto const &each : ctx.arguments) {
            output += (each + " ");
        }
        ctx.channel->send(output).wait();
    });

    bot.register_command("presence", [](discord::Context ctx) {
        ctx.bot->update_presence(discord::Activity{
            "Shroud is streaming!", discord::presence::activity::streaming,
            discord::presence::status::idle, false, "http://twitch.tv/shroud" });
    });
